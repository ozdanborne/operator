...
func (r *ReconcileAPIServer) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	component, err := render.APIServer(&network.Status.Computed, managementCluster, managementClusterConnection, amazon, tlsSecret, pullSecrets, r.provider == operatorv1.ProviderOpenShift,
		tunnelCASecret)
	if err != nil {
